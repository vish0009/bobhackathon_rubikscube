...
Classification Agent: Extracts templates using Drain3, applies rule-based classification, with LLM fallback for ambiguous cases ✓
Value Assessment Agent: Analyzes access patterns, recency, and assigns retention priority ✓ (Phase 2)
Decision/Policy Agent: Applies compliance rules and policy constraints ✓ (Phase 2)
Execution Agent: Performs retention actions and maintains audit trail ✓ (Phase 2)
Key Features
...
Drain3 extracts unique patterns from logs
Each template is classified once
All matching log lines inherit the template's classification
This dramatically reduces processing time and LLM API costs
Quick Start
Prerequisites
...
Classification: Extract templates and classify
Value Assessment: Score each template's retention value
Decision Making: Apply policy rules and compliance overrides
Execution: Perform actions and log to audit trail
Roadmap
Phase 1 ✓ COMPLETE
